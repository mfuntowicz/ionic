...
extern struct ionic_barrier *ionic_barrier_open(struct ionic_context *ctx, const char *identifier);
...
static int test_barrier_size(void) {
    if (sizeof(ionic_barrier_t) != 64)
...
static int test_barrier_multi_process(void) {
    ...
    for (unsigned char i = 1; i < n_procs; i++) {
        ...
        if (pid == 0) {
            ...
            ionic_context_init(&child_ctx, ionic_cpu_device(0), NULL);
            struct ionic_barrier *joined = ionic_barrier_open(&child_ctx, test_shm_name);
            if (!joined) {
...
static int test_barrier_open_sees_initialized_fields(void) {
    make_shm_name();
...
int main(void) {
    ...
    if (test_barrier_multi_process() != IONIC_RESULT_SUCCESS)
        return IONIC_RESULT_FAILURE;
    if (test_barrier_open_sees_initialized_fields() != IONIC_RESULT_SUCCESS)
        return IONIC_RESULT_FAILURE;
