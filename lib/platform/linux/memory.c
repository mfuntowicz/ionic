...
static void *ionic_linux_host_mmap_allocate_impl(size_t size, enum ionic_allocation_kind kind) {
    (void)kind;
    if (size == 0u)
        return NULL;
    ...
    size_t need = ionic_host_hdr_user_off + size;
    if (need < size)
        return NULL;

    size_t map_sz = ionic_align_up_sz(need, huge_2mb);
    if (map_sz == 0u)
        map_sz = need;
    ...
    p = mmap(NULL, map_sz, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB | MAP_HUGE_2MB, -1, 0);
#endif

    if (p == MAP_FAILED) {
        if ((map_sz = ionic_align_up_sz(need, page)) == 0u)
            return NULL;

        if ((p = mmap(NULL, map_sz, PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0)) == MAP_FAILED)
            return NULL;
#ifdef MADV_HUGEPAGE
    ...
    struct ionic_host_map_hdr *hdr = p;
    hdr->magic = IONIC_HOST_MAP_MAGIC;
    hdr->reserved = 0u;
    hdr->map_bytes = map_sz;
    if (ionic_host_hdr_user_off > sizeof(struct ionic_host_map_hdr))  // todo(mfuntowicz) 32-bits system, do we really need it?
